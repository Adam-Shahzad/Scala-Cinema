...
class Movies(newOrCurrent:Int){
  private def movies:List[Map[String,String]] = {
    newOrCurrent match {
      case 0 => {
        scala.util.parsing.json.JSON.parseFull(scala.io.Source.fromURL("https://api.themoviedb.org/3/movie/now_playing?api_key=0e1152c2ffcd8427363a93caf8553082&language=en-UK&page=1").mkString).
          get.asInstanceOf[Map[String, String]].get("results").get.asInstanceOf[List[Map[String, String]]]
      }
      case 1 => {
        scala.util.parsing.json.JSON.parseFull(scala.io.Source.fromURL("https://api.themoviedb.org/3/movie/upcoming?api_key=0e1152c2ffcd8427363a93caf8553082&language=en-UK&page=1").mkString).
          get.asInstanceOf[Map[String, String]].get("results").get.asInstanceOf[List[Map[String, String]]]
      }
    }
  }
  ...
  def genreIDs(value:Int):String = movieList(value).getOrElse("genre-ids".mkString,"No Value")
  ...
  def length:Int = movieList.length
}
